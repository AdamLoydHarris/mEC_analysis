...
def generate_population(num_neurons=50):
    ...
    """
    all_place_cells = []
    used_peak_sets = set()  # to store each set of peaks as a tuple for uniqueness
    ...
    for i in range(num_neurons):
        ...
        while True:
            ...
            # Sort them so we can have a canonical representation
            peak_locations_sorted = tuple(sorted(peak_locations))

            # Check if we've already used this set
            if peak_locations_sorted not in used_peak_sets:
                # It's a new, unique set of peaks => accept it
                used_peak_sets.add(peak_locations_sorted)
                break
            else:
                # It's already used => choose again
                continue

        # Optionally add variability in peak_rate and baseline_rate
        peak_rate = np.random.uniform(5, 15)   # random peak 5-15
        baseline_rate = np.random.uniform(0.5, 2)  # random baseline 0.5-2

        pc = PlaceCell(
            neuron_id=i,
            peak_locations=list(peak_locations_sorted),  # store as list for easy usage
            peak_rate=peak_rate,
